...
interface Props {atlas:Atlas;state:SceneState;partLabel:(part:Part)=>string;onSelect:(id:string)=>void;onProgress:(n:number)=>void;onError:(s:string)=>void}
export default function AnatomyScene({atlas,state,partLabel,onSelect,onProgress,onError}:Props){
 const host=useRef<HTMLDivElement>(null),latest=useRef(state),select=useRef(onSelect),partLabelRef=useRef(partLabel);
 latest.current=state;select.current=onSelect;partLabelRef.current=partLabel;
 useEffect(()=>{
