...
/** Canonical system catalogue: renderer order and colors plus English source text for names and descriptions. The UI resolves Chinese via translate(). */
...
export interface SystemInfo {id:SystemId;name:string;color:string;description:string}
/** One mesh part of a named structure. `name` is the canonical English label from atlas.json; per-locale display labels are injected at runtime by `applyStructureNames` and read via `nameFor` (app/languages). */
...
/** Curated explanations for landmark structures, keyed by the canonical English concept name (lower-case). */
...
/** Whether a curated explanation exists for a canonical structure name; decides if the generic context note is shown. */
export function hasExplanation(name:string):boolean{return name.toLowerCase() in EXPLANATIONS;}
/** English body text for the detail panel: the curated paragraph when present, otherwise the enclosing system description. */
export function explanationSource(name:string,system:SystemId):string{return EXPLANATIONS[name.toLowerCase()] ?? (SYSTEMS.find(s=>s.id===system)?.description ?? '');}
